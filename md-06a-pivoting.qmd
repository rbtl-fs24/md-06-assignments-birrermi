...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.
    ...
    *The tidyverse package is a package containing all subpackages(ggplot,dplyr,...) from tidyverse. It helps to manage and organize data more easily.*
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report?
    ...
    *I would change it back to the wide format for better visualisation.*
...
```{r}
waste_data_wide_mean <- waste_data_long_mean |> 
